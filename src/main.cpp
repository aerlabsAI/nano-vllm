...
public:
    ...
    Tokenizer(const std::string& path, int vocab_size) : vocab_size(vocab_size) {
        load(path);
    }
...
public:
    Sampler(int vocab_size, float temp, float topp, unsigned long long seed)
        : vocab_size(vocab_size), temperature(temp), topp(topp), rng(seed) {}
    ...
    int sample(float* logits) {
        ...
        if (topp > 0.0f && topp < 1.0f) {
            ...
            struct ProbIndex { float p; int i; };
            ...
            for(int i=0; i<=last_idx; i++) {
                cdf += probs[i].p;
                if (r_scaled < cdf) return probs[i].i;
            }
...
public:
    ...
    void forward(int token, int pos) {
        ...
        for (int i = 0; i < config.n_layers; i++) {
            ...
            // RoPE
            Ops::apply_rope(state.q.data(), state.k.data(), pos, config.head_dim, config.n_heads, config.n_kv_heads, config.rope_theta);
            ...
            // Residual
            for(int j=0; j<config.dim; j++) state.x[j] += state.xb[j];
...
private:
    ...
    void read_weights(std::ifstream& file) {
        ...
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].rms_att_weight);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].wq);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].wk);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].wv);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].wo);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].rms_ffn_weight);
        
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].w_gate);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].w_down);
        for(int i=0; i<config.n_layers; i++) read_tensor(weights.layers[i].w_up);
...
int main(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (arg == "-t" && i + 1 < argc) temperature = std::stof(argv[++i]);
        else if (arg == "-p" && i + 1 < argc) topp = std::stof(argv[++i]);
        else if (arg == "-n" && i + 1 < argc) steps = std::stoi(argv[++i]);
        else if (arg == "-i" && i + 1 < argc) prompt = argv[++i];
    }
    ...
    // Decode
    long start_time = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    long end_time = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
